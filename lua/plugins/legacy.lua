...
return {
	-- Everything else!!!
	{ "arthurxavierx/vim-caser", event = LazyFileEvents }, -- Change word casing with vim motion (gs motion)
	...
	{ "kshenoy/vim-signature", event = LazyFileEvents }, -- Used to add/remove/go-to marks, actually pretty cool
	...
	{ "pgr0ss/vim-github-url", event = LazyFileEvents }, -- getter for github urls

	{ "tpope/vim-projectionist" },  -- this is the actual plugin that does the jumping between alternate files base on config
	-- { "tpope/vim-fugitive", event = LazyFileEvents },
	-- { "tpope/vim-ragtag", event = LazyFileEvents },
	{
		"tpope/vim-rake", -- this is pretty handy for jumping between source and test files in a normally structured rails repo
		keys = {
			{ "<Leader>AA", "<cmd>A<CR>", desc = "Alternate file" }, -- TODO: figure out why this doesn't work sometimes
  ...
  { event = LazyFileEvents, "bkad/CamelCaseMotion" },
	{ "romainl/vim-qf", lazy = true },

	-- { "machakann/vim-swap" }, -- This is actually kinda cool but conflicts with other gs- prefixed mappings
	{ "wellle/targets.vim" },
